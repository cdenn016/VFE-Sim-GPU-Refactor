...
\begin{abstract}
During the past decade transformers have achieved remarkable success in language, image, video generation and reasoning, yet their theoretical foundations remain obscure. In previous theoretical work, we derived transformer attention and feed-forward mechanisms from first principles using gauge-equivariant variational free energy defined on a principal bundle. Here, we present a proof-of-principle implementation of this framework, demonstrating that explicit probabilistic inference (without neural architectures) can achieve performance comparable to standard architectures at small scale.

We implement and validate three architectures of increasing theoretical fidelity: (1) standard dot-product attention baseline, (2) KL divergence-based gauge attention with standard feed-forward and backpropagation, and (3) complete gauge-theoretic transformers using natural gradient descent on statistical manifolds with multi-head attention defined by Lie group structure. On character-level language modeling (WikiText-2, context length 32, embedding dimension 11), the full gauge architecture achieves 20\% lower perplexity (PPL 18.06 vs 22.6) than the standard baseline while using 25\% fewer parameters. Our architecture contains zero MLPs, activation functions, or learned weight matrices.

This proof-of-principle establishes that gauge-theoretic attention is mathematically feasible and shows promising results on a small-scale benchmark. Extending to higher-dimensional gauge groups SO(N), we find that SO(20) at embedding dimension 100 achieves PPL 166 with uniform attention---suggesting that semantic structure lives in gauge frames rather than attention weights. While our implementation incurs substantial computational overhead compared to optimized dot-product attention, these preliminary results motivate further investigation into interpretable and geometrically-grounded approaches to attention mechanisms. The extent to which these findings generalize to larger scales and word-level modeling remains an open question for future work.
\end{abstract}
...
Full details of our gauge theoretic geometry may be found in \citet{Dennis2025}. Briefly, agents are modeled as smooth sections of an associated bundle $\mathcal{E}$ to a principal $G$ bundle with statistical fibers $\mathcal{B}$. In our present consideration the fibers,  $\mathcal{B}$, are statistical manifolds of the exponential family of multi-variate Gaussians (MVG) $q_i(c)$, $p_i(c)$ with $K$-dimensional irreducible representations of the structure group $G$ acting on statistics $\mu_q(c)$ and $\Sigma_q(c)$ as
...
Given the action of the group $G$ on the statistical fibers we are able to derive (from a simple generative model~\citep{Dennis2025}) a generalized variational free energy functional at a single point $c \in \mathcal{C}$ as
...
\label{eq:free_energy_simplified}
\end{equation}
...
\subsection{Extension to SO(N) Gauge Groups}
\label{sec:soN}

While our initial experiments use SO(3) as the gauge group (with $\dim(\mathfrak{so}(3)) = 3$ heads), the framework naturally extends to higher-dimensional orthogonal groups SO(N) with $\phi_{\text{dim}} = N(N-1)/2$ gauge directions. This extension reveals an important distinction between two types of generators that live in different spaces.

\subsubsection{Transport vs. Gauge Generators}

For a general SO(N) gauge group acting on embedding space $\mathbb{R}^K$, we must distinguish:

\begin{itemize}
  \item \textbf{Transport generators} $T_a \in \mathbb{R}^{K \times K}$: Act on embedding vectors $\mu \in \mathbb{R}^K$ via the representation $\rho: \text{SO}(N) \to \text{GL}(K, \mathbb{R})$. These define how gauge rotations transform the statistical parameters.

  \item \textbf{Gauge generators} $G_a \in \mathbb{R}^{N \times N}$: The fundamental generators of $\mathfrak{so}(N)$ satisfying the Lie bracket $[G_a, G_b] = f_{abc} G_c$. These define the intrinsic structure of the gauge algebra.
\end{itemize}

For SO(3), these coincide when $K = 3$ (the fundamental representation). However, for general SO(N) with $K \neq N$, they differ crucially. The Baker-Campbell-Hausdorff (BCH) composition for gauge frames uses the Lie bracket in $\mathfrak{so}(N)$:

\begin{equation}
\phi_1 \circ \phi_2 \approx \phi_1 + \phi_2 + \frac{1}{2}[\phi_1, \phi_2]_{\mathfrak{so}(N)} + \cdots
\end{equation}

where the bracket is computed via matrix commutator on the $N \times N$ gauge generators:

\begin{equation}
[\phi_1, \phi_2]_{\mathfrak{so}(N)} = \sum_{a,b,c} \phi_1^a \phi_2^b f_{abc} \, e_c, \quad [G_a, G_b] = G_a G_b - G_b G_a = \sum_c f_{abc} G_c.
\end{equation}

\subsubsection{SO(N) Retraction with Trust Region}

For multi-step variational inference, gauge frame updates $\phi \to \phi + \delta\phi$ must respect the Lie group structure. We implement retraction on SO(N) with trust region constraints to prevent gradient explosion:

\begin{equation}
\phi_{\text{new}} = \phi \circ (\alpha \cdot \delta\phi), \quad \alpha = \min\left(1, \frac{\tau \|\phi\|}{\|\delta\phi\| + \epsilon}\right)
\label{eq:soN_retraction}
\end{equation}

where $\tau \in (0,1)$ limits relative change per update (we use $\tau = 0.3$) and $\circ$ denotes BCH composition in $\mathfrak{so}(N)$. This ensures stable optimization even with many VFE steps.

\subsubsection{Higher-Dimensional Gauge Structure}

For SO(N), the number of gauge directions grows quadratically:

\begin{center}
\begin{tabular}{ccc}
\toprule
\textbf{Gauge Group} & $\phi_{\text{dim}}$ & \textbf{Semantic Capacity} \\
\midrule
SO(3) & 3 & Low \\
SO(10) & 45 & Medium \\
SO(20) & 190 & High \\
SO(50) & 1225 & Very High \\
\bottomrule
\end{tabular}
\end{center}

Higher-dimensional gauge groups provide richer semantic structure in the gauge frames $\phi_i$, potentially capturing more nuanced relationships between agents. This motivates our experimental investigation of SO(10) and SO(20) gauge groups.
...
Table~\ref{tab:main_results} presents our primary result: all three architectures achieve comparable performance on character-level language modeling. At this small scale (context 32, embedding 11, character-level), the full gauge-theoretic transformer (Architecture 3) achieves 20\% lower perplexity than the standard baseline with 25\% fewer parameters.

\begin{table}[t]
\centering
\caption{Test set performance on WikiText-2 character-level modeling (context 32, embedding 11). Metrics are means over 5 independent runs with different random seeds; standard deviations in parentheses. At this scale, the full gauge VFE achieves 20\% lower perplexity than the standard baseline while using 25\% fewer parameters.}
...
\subsection{SO(N) Gauge Group Experiments}
\label{sec:soN_results}

To investigate whether higher-dimensional gauge groups provide better semantic representation, we conducted experiments with SO(10) and SO(20) gauge groups at larger embedding dimensions. Table~\ref{tab:soN_results} presents results with 1 VFE step.

\begin{table}[h]
\centering
\caption{Performance of higher-dimensional gauge groups (1 VFE step). Higher gauge dimensions with larger embedding spaces achieve substantially better perplexity. The best result (SO(20), $K$=100) achieves PPL 166, demonstrating that semantic structure benefits from richer gauge representations.}
\label{tab:soN_results}
\begin{tabular}{lccc}
\toprule
\textbf{Gauge Group} & \textbf{Embed Dim} $K$ & $\phi_{\text{dim}}$ & \textbf{PPL} $\downarrow$ \\
\midrule
SO(3) & 11 & 3 & 18.06 \\
\midrule
SO(10) & 50 & 45 & 341 \\
...
\begin{enumerate}
  \item \textbf{Embedding dimension matters:} For both SO(10) and SO(20), performance improves dramatically as embedding dimension $K$ increases. SO(20) at $K$=20 performs poorly (PPL 410), but at $K$=100 achieves PPL 166.

  \item \textbf{SO(20) outperforms SO(10):} At matched embedding dimension ($K$=100), SO(20) achieves better perplexity than SO(10) (166 vs 180), suggesting that richer gauge structure captures more semantic information.

  \item \textbf{Uniform attention suffices:} Remarkably, these results use uniform attention weights ($\beta_{ij} = 1/N$) rather than learned or KL-based attention. The semantic structure lives in the gauge frames $\phi_i$, not in the attention weights.
\end{enumerate}

The final observation is particularly surprising: with sufficiently rich gauge representations, explicit attention computation may be unnecessary. Analysis of the gauge frames reveals that different token types cluster in distinct regions of the $\phi$-space, with intra-class distances approximately 1.54$\times$ smaller than inter-class distances. This suggests that gauge frames naturally encode semantic similarity, making attention weight optimization redundant.
...
This work demonstrates that gauge-theoretic transformers without neural networks are feasible and can achieve competitive performance on small-scale character-level language modeling. At this scale, our full gauge VFE achieves 20\% lower perplexity (PPL 18.06 vs 22.6) than the standard baseline while using 25\% fewer parameters (6,534 vs 8,688), with train-validation gaps within $\pm 0.1$ BPC indicating controlled generalization.

Our empirical results establish five key findings:
...
\begin{enumerate}
  ...
  \item \textbf{Higher-dimensional gauge groups} (SO(10), SO(20)) capture richer semantic structure, with SO(20) at embedding dimension 100 achieving PPL 166

  \item \textbf{Uniform attention suffices} when gauge representations are rich---semantic structure lives in $\phi$, not in $\beta$
\end{enumerate}
...
\subsubsection{Why Uniform Attention Works}

Our SO(N) experiments reveal a surprising finding: with sufficiently rich gauge representations, uniform attention ($\beta_{ij} = 1/N$) achieves competitive performance. This challenges the conventional view that learned attention weights are essential for transformer performance.

\textbf{Semantic structure in $\phi$, not $\beta$.} Analysis of trained gauge frames reveals that tokens cluster by semantic similarity in $\phi$-space. We observe:

\begin{itemize}
  \item Intra-class $\phi$ distances are approximately 1.54$\times$ smaller than inter-class distances
  \item Similar characters (e.g., vowels vs consonants, uppercase vs lowercase) occupy nearby regions in the gauge manifold
  \item This clustering emerges without explicit supervision---it arises from variational free energy minimization
\end{itemize}

This suggests that gauge frames serve as implicit ``soft attention'' through parallel transport: when $\phi_i \approx \phi_j$, the transport operator $\Omega_{ij} \approx I$ preserves beliefs, while dissimilar frames induce rotations that effectively downweight contributions from unrelated tokens.

\textbf{Implications for standard transformers.} If semantic structure can be encoded entirely in coordinate frames rather than attention weights, this suggests that learned attention in standard transformers may partially serve to approximate gauge-theoretic transport. The success of uniform attention with rich gauge structure provides evidence that attention weight computation may be a computational convenience rather than an essential mechanism.

However, we note that these findings are preliminary and require validation at larger scales. The relationship between $\phi_{\text{dim}}$, embedding dimension $K$, and the importance of attention weights remains an open question.
...
We have presented a proof-of-principle demonstration of an attention transformer based on gauge-theoretic variational free energy that achieves 20\% lower perplexity on character-level prediction than a comparable standard transformer baseline, using 25\% fewer parameters. At this small scale (context 32, embedding 11, character-level WikiText-2), our framework performs competitively without learned MLP weights, with non-linear transformations emerging from variational free energy belief dynamics, position information encoded via gauge frames, and multi-head attention structured by Lie group generators.

Furthermore, extending to higher-dimensional gauge groups reveals that SO(20) at embedding dimension 100 achieves PPL 166 with uniform attention weights. This surprising finding suggests that semantic structure is encoded in gauge frames $\phi_i$ rather than attention weights $\beta_{ij}$---the gauge manifold naturally clusters similar tokens, making explicit attention computation potentially redundant when gauge representations are sufficiently rich.

However, we emphasize important limitations. First, the computational overhead is extreme (825$\times$ slower than standard attention) due to unoptimized matrix exponentials and KL divergence computations. Second, our experiments are at very small scale---whether these results generalize to word-level modeling, larger embedding dimensions, or longer contexts remains an open question. Third, we have not compared against well-tuned baselines or state-of-the-art character-level models.

The primary contribution of this work is demonstrating feasibility: transformers can be implemented without neural network components through geometric inference on statistical manifolds. Future work includes scaling to larger models and datasets, implementing computational optimizations to reduce overhead, exploring learned irrep decompositions to discover symmetries in data, investigating whether standard transformers implicitly approximate gauge-theoretic operations, and understanding the interplay between gauge dimension and attention weight importance. These preliminary results suggest gauge-theoretic foundations may offer new perspectives on transformer mechanisms, though substantial further validation is required.
