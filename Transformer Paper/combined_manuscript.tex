...
\subsection{Backpropagation as Free Energy Gradient Descent}

In the deterministic limit, the free energy becomes:
\begin{equation}
\mathcal{F}[\{\mu_i\}] = \sum_i \frac{\lambda_p}{2}\|\mu_i - \mu_{\text{prior}}\|^2 + \sum_{i,j} \frac{\beta_{ij}}{2\sigma^2}\|\mu_i - \mu_j\|^2 - \sum_i \log p(o_i | \mu_i).
\end{equation}

The gradient flow $\frac{d\mu_i}{dt} = -\eta \frac{\partial \mathcal{F}}{\partial \mu_i}$ is \textbf{mathematically identical} to backpropagation:
\begin{equation}
...
This establishes that standard neural network training is the deterministic limit of variational free energy minimization.
...
We implement the full non-degenerate framework as a working transformer for language modeling.

\subsection{Architecture}

Each token $i$ at position $t$ maintains:
\begin{itemize}
\item \textbf{Belief mean:} $\mu_i \in \mathbb{R}^K$ (initialized from input embedding)
\item \textbf{Belief covariance:} $\Sigma_i \in \mathbb{R}^{K \times K}$, $\Sigma_i \succ 0$
\item \textbf{Gauge frame:} $\phi_i \in \mathfrak{so}(3)$ (encodes position)
\end{itemize}

\textbf{No MLPs, no learned weight matrices, no activation functions.}
...
\item Backpropagation equals variational gradient descent in this limit
\item The full framework can be implemented as a working transformer
\item Empirical validation confirms the theoretical equivalence
\end{enumerate}
...
\item Proves that backpropagation is variational gradient descent in the deterministic limit
