...
The most striking empirical finding is the dramatic difference in convergence speed between VFE and standard transformers. Under identical configurations (embedding dimension $K=19$, context length $N=256$, vocabulary 50,257):

\begin{table}[h]
\centering
\caption{Convergence comparison on WikiText-103 BPE.}
\begin{tabular}{lcccc}
\toprule
\textbf{Model} & \textbf{Steps to PPL $<$300} & \textbf{It/sec} & \textbf{Wall-clock} & \textbf{Best PPL} \\
\midrule
Standard Transformer & $>$4500 (not reached) & 115 & $>$39s & 543 (step 4500) \\
Gauge VFE Transformer & $\sim$100 & 10 & $\sim$10s & 273 \\
\bottomrule
\end{tabular}
\end{table}

The VFE transformer achieves target perplexity in approximately 45 times fewer optimization steps. Despite being 11 times slower per iteration (due to full KL computation and natural gradient dynamics), the VFE model reaches better perplexity in roughly 4 times less wall-clock time.

At step 4500, the standard transformer's samples remain incoherent: ``\textit{This that was for..... million and the- . was a at . of that was of and in with.....,...}''---demonstrating that it has not yet learned basic syntactic structure. In contrast, the VFE transformer produces coherent text within 100 steps.

\subsubsection{Interpretation: Natural Gradient Advantage}

This convergence speedup is predicted by information geometry. VFE dynamics use natural gradient descent on the Fisher metric of the belief manifold, which offers exponential convergence $O(\log(1/\varepsilon))$ versus Euclidean gradient's $O(1/\varepsilon)$ rate. The Fisher metric accounts for the local curvature of the probability distribution, taking larger steps in flat directions and smaller steps in curved directions.

For Gaussian beliefs with covariance $\Sigma$, the natural gradient is:
\begin{equation}
\nabla_{\text{nat}} \mathcal{F} = \Sigma \cdot \nabla_\mu \mathcal{F}
\end{equation}
which preconditions the Euclidean gradient by the Fisher information matrix. This preconditioning is particularly effective in high-dimensional spaces where Euclidean gradients suffer from ill-conditioning.

\begin{table}[h]
\centering
\caption{WikiText-103 BPE results (K=19, N=256, vocab=50,257).}
\begin{tabular}{lccc}
\toprule
\textbf{Model} & \textbf{Val PPL} & \textbf{Test PPL} & \textbf{Parameters} \\
\midrule
Standard Transformer & [PENDING] & [PENDING] & [PENDING] \\
Gauge VFE Transformer & 273 & [PENDING] & [PENDING] \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Natural gradient speedup.} Our framework uses natural gradient descent on the Fisher metric, which offers exponential convergence $O(\log(1/\varepsilon))$ versus Euclidean gradient's $O(1/\varepsilon)$. This theoretical advantage manifests dramatically in practice: the VFE transformer reaches target perplexity in $\sim$45$\times$ fewer steps than standard gradient descent (Section~\ref{sec:experiments}).
...
Thus KL divergence scales linearly with embedding dimension $K$.
...
In practice, if using fixed per-dimension variance (common after training), one may set $\kappa \approx K/K_{\mathrm{ref}}$ where $K_{\mathrm{ref}}$ is a reference dimension at which $\kappa=1$ yields stable training.
